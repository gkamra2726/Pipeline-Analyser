                        ...
                        VOID* disasPtr)
{
    ...
    // ── 1. RAW Data Hazard — NO forwarding ───────────────────────────────────
    auto checkReg = [&](REG r) -> UINT64 {
    ...
    UINT64 s0 = checkReg(read0);
    UINT64 s1 = checkReg(read1);
    dStall = (s0 > s1) ? s0 : s1;

    // ── 2. Load-Use hazard ────────────────────────────────────────────────────
    if (dStall == 0) {
        auto checkLoadUse = [&](REG r) -> bool {
            if (r == REG_INVALID()) return false;
            auto it = isLoadReg.find(r);
            if (it == isLoadReg.end() || !it->second) return false;
            auto rc = registerReadyCycle.find(r);
            if (rc != registerReadyCycle.end() && rc->second == currentCycle)
                return true;
            return false;
        };
        if (checkLoadUse(read0) || checkLoadUse(read1)) {
            loadUse = true;
            loadUseStalls++;
            dStall = 1;
        }
    ...
    if (instrLog.size() < MAX_LOG) {
        ...
        entry.forwarded = false;   // no forwarding in this version
